...
with open("data_config.csv", newline="", encoding="utf-8") as csvfile:
    ...
    for row in reader:
        ...
        for i in range(3):
            ...
            # Prepare the glove mask for the low-resolution image and high-resolution image
            os.system(
                f"python ./data_process/segment_util_image.py --img_path {output_path}/{case_name}/{i}.png --TEXT_PROMPT 'glove' --output_path {output_path}/{case_name}/mask_glove_{i}.png"
            )
            os.system(
                f"python ./data_process/segment_util_image.py --img_path {output_path}/{case_name}/{i}_high.png --TEXT_PROMPT 'glove' --output_path {output_path}/{case_name}/mask_glove_{i}_high.png"
            )
