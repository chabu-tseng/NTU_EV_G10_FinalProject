...
def render_multi_images(
    ...
    # Render four times to avoid memory overflow
    split = num_cameras // 4
    color1, depth1 = render_image(
        mesh, camera_poses[:split], width, height, fov, device
    )
    color2, depth2 = render_image(
        mesh, camera_poses[split:2*split], width, height, fov, device
    )
    color3, depth3 = render_image(
        mesh, camera_poses[2*split:3*split], width, height, fov, device
    )
    color4, depth4 = render_image(
        mesh, camera_poses[3*split:], width, height, fov, device
    )
    color = np.concatenate([color1, color2, color3, color4], axis=0)
    depth = np.concatenate([depth1, depth2, depth3, depth4], axis=0)
    return color, depth, camera_poses, camera_intrinsics
