...
parser.add_argument('--model_type', default='DPT_Large', type=str)
...
for dataset_id in scenes:
    ...
    if args.benchmark=="DTU":
        root_path_1 = root_path+dataset_id
    ...
    for image_paths in image_path_pkg[0]:
        jpg_files = glob.glob(os.path.join(image_paths, '*.jpg'))
        JPG_files = glob.glob(os.path.join(image_paths, '*.JPG'))
        png_files = glob.glob(os.path.join(image_paths, '*.png'))
        PNG_files = glob.glob(os.path.join(image_paths, '*.PNG'))
        img_files = jpg_files + JPG_files + png_files + PNG_files
        
        for filename in img_files:
            img = cv2.imread(filename)
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

            print('filename, img.shape:', filename.split('/')[-1], img.shape) #(1213, 1546, 3)
            h, w = img.shape[:2]
