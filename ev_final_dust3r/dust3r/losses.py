...
#         in_camera1 = torch.tensor([
#             [ 0.09272623, -0.7038759,   0.70424485,  0.2046381 ],
#             [ 0.9956918,   0.06555015, -0.06558463,  0.22353707],
#             [ 0.0,         0.7072921,   0.70692146,  1.2924082 ],
#             [ 0.0,         0.0,         0.0,         1.0       ]], dtype=torch.float32)
#         in_camera1 = inv(in_camera1)
#         #print("gt1 keys:", gt1.keys())
#         gt1_pts = gt1['pts3d_seq']  # B,H,W,3
#         #print(gt1_pts.shape)  
#         B,F,H,W,C = gt1_pts.shape
#         in_camera1 = in_camera1[None, ...].expand(B , 4, 4).to(gt1_pts.device)
#         gt_pts1 = geotrf(in_camera1, gt1_pts)  # B,H,W,3
        
#         #gt_pts2 = geotrf(in_camera1, gt2['pts3d'])  # B,H,W,3

#         valid1 = torch.ones(gt_pts1.shape[:-1], dtype=torch.bool, device=gt_pts1.device)
#         # valid2 = torch.ones(gt_pts2.shape[:-1], dtype=torch.bool, device=gt_pts2.device)
...
class Regr3D (Criterion, MultiLoss):
    ...
    def get_all_pts3d(self, gt1, pred1, dist_clip=None):
        # everything is normalized w.r.t. camera of view1
        gt1_pts = gt1['gt_pts3d']  # B,H,W,3
        gt1_colors = gt1['gt_colors']  # B,H,W,3
        valid1 = gt1['mask']  # B,H,W
        #B,F,H,W,C = gt1_pts.shape
        # pred_pts = pred1[0]  # B,H,W,3
        pred_colors = pred1
       # print(f"\n\n\n\n\npred s[hape_pts,color",pred_pts.shape, pred_colors.shape)  # B,H,W,3
        #print(gt1_pts.shape)
        # gt_pts1 = geotrf(in_camera1, gt1_pts)  # B,H,W,3        
        #gt_pts2 = geotrf(in_camera1, gt2['pts3d'])  # B,H,W,3
        #alid1 = torch.ones(gt1_pts.shape, dtype=torch.bool, device=gt1_pts.device)
        #print(pred1.keys())
        #pr_pts1 = get_pred_pts3d(gt1, pred1, use_pose=False)
        # pr_pts2 = get_pred_pts3d(gt2, pred2, use_pose=True)
        ...
        return gt1_pts, gt1_colors , None, pred_colors, valid1, {}

    def compute_loss(self, gt1, gt2, pred1, pred2, **kw):
        gt_pts1, gt_colors1, pred_pts1, pred_colors1, mask1, monitoring = self.get_all_pts3d(gt1, pred1, **kw)
        if isinstance(pred_colors1, dict):
            #print(f"[debug] pred_colors1 keys: {pred_colors1.keys()}")
            pred_colors1 = pred_pts1['pts3d']

        
        #print(gt_pts1.shape, gt_colors1.shape, pred_pts1.shape, pred_colors1.shape, mask1.shape
        # 設定 color loss 權重
        #color_weight = 0.1  # ✅ 調整這個係數來決定顏色誤差在總損失中的重要性
        #mask_color = mask1.unsqueeze(1).expand(-1, 3, -1, -1)  # (B, 3, H, W)
        #print(f"[debug] gt_colors1 shape: {gt_colors1.shape}, pred_colors1 shape: {pred_colors1.shape}")
        #print(f"pred,gtcolorshape",pred_colors1.shape, gt_colors1.shape)
        pred_colors1 = pred_colors1.permute(0,1, 3, 4, 2)  # (B, H, W, C) → (B, C, H, W)
        #gt_colors1 = gt_colors1.permute(0, 3, 1, 2)

        l1_color = self.criterion(pred_colors1, gt_colors1)
        #l1_pts = self.criterion(pred_pts1[mask1], gt_pts1[mask1])

        total_loss = l1_color

        self_name = type(self).__name__
        details = {
            self_name + '_color_1': float(l1_color.mean()),
        }

        return Sum((total_loss, mask1)), (details | monitoring)
